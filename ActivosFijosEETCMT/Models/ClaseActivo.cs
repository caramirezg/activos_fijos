...
namespace ActivosFijosEETC.Models
{
    public class ClaseActivo
    {
        ...
        /// <param name="fkc_tipo_adquisicion"></param>
        /// <returns></returns>
        public int CreaActivoPorTransferencia(string codigo, string correlativo, int fk_fuente_financiamiento, int fk_auxiliar_contable, int fk_modelo, string serie, string descripcion, DateTime f_registro, int fkc_estado_activo, int fkc_estado_proceso, decimal tasa_ufv, decimal tasa_sus, decimal valor_inicial,decimal costo_actualizado_inicial,decimal depreciacion_acumulada_total, int fk_transferencia,  int fkc_tipo_adquisicion)
        {
            using (SqlConnection connection = new SqlConnection(conexion.connectionString))
            {
                connection.Open();
                ...
                try
                {
                    ...
                    command.Parameters.Add("@costo", SqlDbType.Decimal).Value = decimal.Round(valor_inicial, 5);
                    command.Parameters.Add("@costo_actualizado_inicial", SqlDbType.Decimal).Value = decimal.Round(costo_actualizado_inicial, 5);
                    command.Parameters.Add("@depreciacion_acumulada_total", SqlDbType.Decimal).Value = decimal.Round(depreciacion_acumulada_total, 5);
                    ...
                    command.CommandText =
                     "insert into activos " +
                     "(codigo,fk_fuente_financiamiento,fk_auxiliar_contable,correlativo,fk_modelo,serie,descripcion,f_registro,fkc_estado_proceso,tasa_ufv,tasa_sus,costo,gastos_con_credito_fiscal,gastos_sin_credito_fiscal,valor_inicial,fk_transferencia,fkc_tipo_adquisicion,costo_actualizado_inicial_historico,depreciacion_acumulada_total_historico,costo_actualizado_inicial,depreciacion_acumulada_total,activo,usuariocreacion,fechacreacion) " +
                     "values('" + codigo + "'," + fk_fuente_financiamiento + "," + fk_auxiliar_contable + "," + correlativo + "," + fk_modelo + ",'" + serie + "','" + descripcion + "','" + f_registro + "'," + fkc_estado_proceso + ",@tasa_ufv,@tasa_sus,@costo,@gastos_con_credito_fiscal,@gastos_sin_credito_fiscal,@costo," + fk_transferencia + "," + fkc_tipo_adquisicion + ",@costo_actualizado_inicial,@depreciacion_acumulada_total,@costo_actualizado_inicial,@depreciacion_acumulada_total,1,'" + userName + "','" + DateTime.Now + "')";
        ...
        "a.valor_inicial,a.costo_actualizado_inicial_historico,a.depreciacion_acumulada_total_historico, " +
        "a.fkc_tipo_adquisicion, " +
        ...
        "and a.activo=1";
            ...
            List<ActivoEntity> Lista = (from AnyName in dtTable.AsEnumerable()
                                        ...
                                        select new ActivoEntity()
                                        {
                                            ...
                                            tasa_sus = AnyName.Field<decimal>("tasa_sus"),
                                            valor_inicial = AnyName.Field<decimal>("valor_inicial"),
                                            costo_actualizado_inicial_historico = AnyName.Field<decimal>("costo_actualizado_inicial_historico"),
                                            depreciacion_acumulada_total_historico = AnyName.Field<decimal>("depreciacion_acumulada_total_historico")
                                        }).ToList();
            return Lista;
        }
        ...
        public int DepreciacionActivos(DateTime fecha)
        {
            ...
            //    return 0;
            //}
            using (var conn = new SqlConnection(conexion.connectionString))
            {
                conn.Open();
                using (var cmd = new SqlCommand("[p_algoritmo_depreciacion_actualizacion]", conn))
                {
                    int result = 0;
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandTimeout = 180; // Increase this to allow the proc longer to run
                    cmd.Parameters.AddWithValue("@fecha_depreciacion", fecha);
