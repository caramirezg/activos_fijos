...
namespace ActivosFijosEETC.Controllers
{
    ...
    public class ControllerActivos : System.Web.Services.WebService
    {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public int CreaActivoPorTransferencia(string fk_fuente_financiamiento, string fk_auxiliar_contable, string fk_modelo, string serie, string descripcion, string f_registro, string valor_inicial, string fk_transferencia,  string fkc_tipo_adquisicion, string tasa_ufv, string tasa_sus)
        {
            int Result = 0;
            string correlativo = ObjetoActivo.obtieneCorrelativo(int.Parse(fk_fuente_financiamiento), int.Parse(fk_auxiliar_contable));
            string codigo = ObjetoActivo.generaCodigoActivo(int.Parse(fk_fuente_financiamiento), int.Parse(fk_auxiliar_contable), correlativo);
            ...
            int fkc_estado_proceso = 4;//ESTADO ELABORADO
            Result = ObjetoActivo.CreaActivoPorTransferencia(codigo, correlativo, int.Parse(fk_fuente_financiamiento), int.Parse(fk_auxiliar_contable), int.Parse(fk_modelo), serie, descripcion, DateTime.Parse(f_registro), fkc_estado_activo, fkc_estado_proceso, decimal.Parse(tasa_ufv), decimal.Parse(tasa_sus), decimal.Parse(valor_inicial), int.Parse(fk_transferencia), int.Parse(fkc_tipo_adquisicion));
            return Result;
        }
        /// <summary>
